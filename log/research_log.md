...
**测试命令**：`python -m src.evaluate_crosssim --checkpoint checkpoints/caption_transformer_epoch_10.pt --crosssim-checkpoint checkpoints/cmm_crosssim_conditions/caption_transformer_cmm_crosssim_ideal.pt`

```text
=== Evaluation Summary ===
Device: cuda
CMM-CrossSim format: cmm_crosssim_v1
CMM-CrossSim mapped linear layers: 21
CMM-CrossSim scope: decoder_only
CMM-CrossSim tile shape: (128, 128)
CMM-CrossSim Rmin/Rmax: 1000.0/100000.0
CMM-CrossSim cell bits: 0
CMM-CrossSim write/read noise std: 0.0/0.0
CMM-CrossSim ADC/DAC: 0/0
CMM-CrossSim GPU: False
Batches evaluated: 100
Baseline  loss: 2.534256, token_acc: 0.476609
CMM-CrossSim  loss: 2.534256, token_acc: 0.476609
Delta loss (cmm-crosssim - base): 0.000000
Delta acc  (cmm-crosssim - base): 0.000000
Logit MAE: 0.000001
Logit Max Error: 0.000072
Logit RMSE: 0.000001
```
...
- 理想条件下，**CMM 映射到忆阻器 CrossSim 后仍保持与 baseline 完全一致**：loss / token_acc 无差异，logit 误差仅为 `1e-6` 量级
- `decoder_only` 范围下 **21 个线性层全部成功映射**，CMM-CrossSim checkpoint 元数据（tile shape、Rmin/Rmax、cell bits、ADC/DAC、noise）完整可读
- 这说明当前 `cmm_crosssim_v1` 映射链路在 **理想器件、理想 ADC/DAC、无噪声** 条件下数值正确，已经具备作为后续 CMM-CrossSim 非理想消融实验的基线资格
- 与前面的 PyTorch `CMMLinear` 理想验证共同构成两级基线：前者验证 **CMM 等效模型本身正确**，这里进一步验证 **CMM → CrossSim 忆阻器映射链路正确**

### CMM-CrossSim ADC 分辨率消融

**数据来源**：`checkpoints/cmm_crosssim_adc_conditions/metrics_pycoco.json`

**条件说明**：这是 **使用 CrossSim 映射后的 CMM 模型** 的 ADC sweep，固定 `decoder_only`、`tile_shape=128×128`、`num_mapped_linear=21`，并在 `write_noise_std=1e-4`、`read_noise_std=1e-4` 条件下扫描 ADC 分辨率。

| 条件 | adc_resolution | BLEU-1 | BLEU-4 | METEOR | ROUGE-L | CIDEr | SPICE |
| --- | --- | --- | --- | --- | --- | --- | --- |
| baseline | — | 0.6816 | 0.2464 | 0.2276 | 0.4954 | 0.8546 | 0.1680 |
| **adc-12bit** | 12 | 0.6808 | 0.2509 | 0.2292 | 0.4975 | 0.8594 | 0.1683 |
| **adc-10bit** | 10 | 0.6640 | 0.2434 | 0.2231 | 0.4845 | 0.8383 | 0.1641 |
| adc-8bit | 8 | 0.6445 | 0.2166 | 0.2244 | 0.4825 | 0.7542 | 0.1600 |
| adc-6bit | 6 | 0.2922 | 0.0287 | 0.1168 | 0.2820 | 0.0722 | 0.0634 |
| adc-4bit | 4 | 0.0615 | ~0 | 0.0132 | 0.0950 | ~0 | 0.0000 |

**分析**：

- **adc-12bit 基本无损**：各项指标与 baseline 持平，BLEU-4/CIDEr 甚至略高，说明在轻微读写噪声条件下，12-bit ADC 足以保持 CMM-CrossSim caption 质量
- **adc-10bit 已出现可见退化**：BLEU-4 从 0.2464 降至 0.2434，CIDEr 从 0.8546 降至 0.8383（约 -1.9%），说明 CMM 经 CrossSim 映射后对 ADC 的敏感性已经明显高于纯 PyTorch CMM 等效模型
- **adc-8bit 明显退化**：BLEU-4 降至 0.2166，CIDEr 降至 0.7542，caption 质量出现系统性下降
- **adc-6bit 接近失效**：BLEU-4 仅 0.0287，CIDEr 仅 0.0722，模型已经基本失去有效描述能力
- **adc-4bit 功能崩溃**：BLEU-4 ≈ 0，CIDEr ≈ 0，SPICE = 0，说明低精度 ADC 会在 CrossSim 器件路径下引发灾难性误差累积
- **与纯 CMM ADC 消融相比，CMM-CrossSim 明显更敏感**：纯 PyTorch CMM 中 adc-10/8 仍较稳定，而在 CrossSim 映射后 adc-10 已开始退化、adc-8 已明显下降，说明真实 crossbar 路径放大了 ADC 量化误差

**核心结论**：

- **推荐 CMM-CrossSim ADC ≥ 10-bit**：12-bit 基本无损，10-bit 仅轻微退化，可作为 nominal 配置
- **adc-8bit 为明显退化起点**，caption 指标已出现系统性下降
- **adc-6bit 以下不可用**：在真实 CrossSim 忆阻器路径下，ADC 很快成为主要精度瓶颈之一
- 这组结果说明：一旦将 CMM 放入 CrossSim 真实器件仿真路径，ADC 敏感性会显著增强，结论更接近早先 CrossSim 原生模型，而不再像纯 PyTorch CMM 那样“温和”

### CMM-CrossSim DAC 分辨率消融

**数据来源**：`checkpoints/cmm_crosssim_dac_conditions/metrics_pycoco.json`

**条件说明**：这是 **使用 CrossSim 映射后的 CMM 模型** 的 DAC sweep，固定 `decoder_only`、`tile_shape=128×128`、`num_mapped_linear=21`，并在 `write_noise_std=1e-4`、`read_noise_std=1e-4` 条件下扫描 DAC 分辨率。

| 条件 | dac_resolution | BLEU-1 | BLEU-4 | METEOR | ROUGE-L | CIDEr | SPICE |
| --- | --- | --- | --- | --- | --- | --- | --- |
| baseline | — | 0.6816 | 0.2464 | 0.2276 | 0.4954 | 0.8546 | 0.1680 |
| **dac-12bit** | 12 | 0.5812 | 0.1614 | 0.1756 | 0.4337 | 0.5380 | 0.1235 |
| **dac-10bit** | 10 | 0.5811 | 0.1627 | 0.1764 | 0.4345 | 0.5405 | 0.1240 |
| dac-8bit | 8 | 0.5786 | 0.1576 | 0.1756 | 0.4340 | 0.5413 | 0.1226 |
| dac-6bit | 6 | 0.5409 | 0.1471 | 0.1674 | 0.4276 | 0.5216 | 0.1205 |
| dac-4bit | 4 | 0.2066 | 0.0420 | 0.0654 | 0.2599 | 0.1713 | 0.0589 |

**分析**：

- **即使 dac-12bit / dac-10bit 也已经显著退化**：BLEU-4 仅约 0.16，CIDEr 仅约 0.54，远低于 baseline，说明在 CMM-CrossSim 路径下，DAC 已经不是“高 bit 即无损”的轻微因素，而是强敏感项
- **dac-12 / dac-10 / dac-8 三组差异很小**：三者 BLEU-4 都在 0.158 ~ 0.163 之间，CIDEr 都在 0.538 ~ 0.541 之间，说明一旦进入当前这条真实器件仿真链路，DAC 误差主导效应已经在高 bit 区间出现平台化
- **dac-6bit 进一步退化**：BLEU-4 降至 0.1471，CIDEr 降至 0.5216，caption 质量继续下降，但尚未完全崩溃
- **dac-4bit 明显接近失效**：BLEU-4 仅 0.0420，CIDEr 仅 0.1713，SPICE 仅 0.0589，说明低精度 DAC 会严重破坏输出分布
- **无读写噪声对照进一步确认 DAC 是主因**：在 `checkpoints/cmm_crosssim_dac_conditions_no_noise/metrics_pycoco.json` 中，`dac-12bit` 且 `write/read noise = 0/0` 时，BLEU-4 仍只有 **0.1637**、CIDEr 仅 **0.5397**，与含 `1e-4` 级别读写噪声时（BLEU-4 **0.1614**、CIDEr **0.5380**）几乎一致，说明当前退化主要由 **输入侧 DAC 量化** 引起，而不是由 `1e-4` 级别读写噪声耦合造成
- **与纯 PyTorch CMM DAC 消融形成鲜明对比**：纯 CMM 中 dac-10/8/6 基本无损，而映射到 CrossSim 后即便 dac-12 也已经大幅掉点，说明真实忆阻器路径中的 DAC 量化误差传播远强于 CMM 等效模型
- **DAC 敏感性甚至强于本轮 CMM-CrossSim ADC sweep**：ADC-12 仍接近 baseline，而 DAC-12 已经出现大幅性能损失，表明在当前 CMM-CrossSim 设置下，DAC 很可能比 ADC 更早成为主要瓶颈

**核心结论**：

- **当前 CMM-CrossSim 路径下，DAC 是极强敏感项**：即使 12-bit / 10-bit 也无法保持 baseline 水平
- **dac-4bit 接近功能失效**，而 `dac-12 ~ dac-8` 虽未崩溃，但整体已处于显著退化区
- **12-bit DAC 在无读写噪声条件下仍显著退化**，进一步证明当前性能下降主要由 **DAC 量化本身** 引起，而不是 `1e-4` 级别读写噪声耦合造成
- 与纯 PyTorch CMM 不同，**CrossSim 映射后的 CMM 对 DAC 的敏感性显著增强**，说明真实 crossbar 路径会强烈放大 DAC 非理想误差
- 这组结果进一步支持你的核心判断：**PyTorch CMM 等效模型过于温和，而一旦进入 CrossSim 真实器件仿真，ADC/DAC 非理想性会迅速成为 caption 质量的主导瓶颈**
