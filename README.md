...
本项目旨在 **将 Transformer 解码器中的线性层映射到 CMM crossbar 上**，通过 PyTorch 级别的误差注入仿真，评估不同写入精度（it-10 ~ it-6）下的模型性能退化，为硬件设计提供指导。
...
| 组件 | 配置 |
|------|------|
| Image Encoder | VGG-16 fc7 / ResNet-50 |
...
**输出**：Ref-Transformer（软件基线模型）

### 阶段二：CMM 误差注入仿真

在 PyTorch 中模拟 CMM 写入误差，不涉及实际电路：
...
测试不同写入条件 `i·dt` 下的 BLEU 变化：

| 写入条件 | 精度等级 |
|----------|----------|
| it-10 (i·dt = 1e-10) | 高精度 |
| it-9 (i·dt = 1e-9) | |
| it-8 (i·dt = 1e-8) | 参考折中点 |
| it-7 (i·dt = 1e-7) | |
| it-6 (i·dt = 1e-6) | 低精度 |

**输出**：各 it 等级下的 BLEU-1~4 退化曲线（对照原论文 Table 2）
...
| **CMM-it10** | CMM 写入条件 i·dt = 1e-10 |
| **CMM-it9** | CMM 写入条件 i·dt = 1e-9 |
| **CMM-it8** | CMM 写入条件 i·dt = 1e-8（参考折中点） |
| **CMM-it7** | CMM 写入条件 i·dt = 1e-7 |
| **CMM-it6** | CMM 写入条件 i·dt = 1e-6 |
| **Mem32-Transformer** | 传统单忆阻器 crossbar 对照 |
...
│   │   ├── decoder.py                 # 多头注意力、解码器层、Transformer 解码器
...
```bash
conda create -n cmm python=3.10
conda activate cmm

pip install torch torchvision --index-url https://download.pytorch.org/whl/cu118
pip install tqdm pycocoevalcap nltk
```
