"""
批量构建不同写噪声强度下的 CrossSim 条件模型。

该脚本先在 PyTorch 权重上注入可控高斯噪声，再将 decoder 线性层转换为
CrossSim AnalogLinear，用于比较不同编程精度条件下的图像描述性能。
"""

# 运行示例（Windows，使用 ^ 续行）：
#   :: 批量构建全部条件（it-10 ~ it-6），含 CrossSim 基线
#   python -m src.test_crosssim_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_conditions --save-baseline-crosssim
#
#   :: 仅构建指定条件子集，跳过已存在的文件
#   python -m src.test_crosssim_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_conditions ^
#       --conditions it-10,it-9,it-8 --skip-existing
#
#   :: 自定义 tile 与量化参数，使用 GPU 加速
#   python -m src.test_crosssim_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_conditions_gpu ^
#       --device cuda --use-gpu --tile-rows 64 --tile-cols 64 ^
#       --cell-bits 8 --adc-resolution 8 --dac-resolution 8
#
#   :: 指定随机种子保证可复现，同时添加读噪声与编程误差
#   python -m src.test_crosssim_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_conditions_noisy ^
#       --seed 123 --read-noise-std 0.01 --programming-error-std 0.005
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="通过注入写噪声构建不同编程精度等级的 CrossSim 条件模型"
    )
    parser.add_argument("--checkpoint", type=Path, required=True, help="原始训练检查点路径")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("checkpoints/crosssim_conditions"),
        help="条件模型输出目录",
    )
    parser.add_argument(
        "--device",
...
def build_condition_models(args: argparse.Namespace) -> List[Dict[str, str]]:
    ...
    for condition, noise_std in conditions.items():
        ...
        if args.skip_existing and out_path.exists():
            ...
            rows.append({"condition": condition, "noise_std": str(noise_std), "checkpoint": str(out_path)})
            continue
...
def main() -> None:
    """主入口：解析参数、构建条件模型、输出 manifest。"""
    args = parse_args()
    rows = build_condition_models(args)
    print("=== Built CrossSim condition models ===")
    for row in rows:
        print(f"{row['condition']}: noise_std={row['noise_std']} -> {row['checkpoint']}")
