"""
批量构建不同 CMM-on-CrossSim write_noise_std 下的条件模型。

固定 array=128x128、ADC/DAC=0/0、cell_bits=0、read_noise_std=1e-4，
只扫描写入噪声。CMM 语义下的 write_noise_std 在 CrossSim 后端映射为
programming_error_std，并在 checkpoint 元数据中明确记录。
"""

# 运行示例（Windows，使用 ^ 续行）：
#   :: 默认 write_noise sweep，每个噪声强度跑 3 个 seed
#   python -m src.test_cmm_crosssim_write_noise_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt
#
...
def parse_args() -> argparse.Namespace:
    """解析 CMM-on-CrossSim write_noise sweep 参数。"""
    parser = argparse.ArgumentParser(description="Build CMM-on-CrossSim checkpoints for write_noise sweep")
    parser.add_argument("--checkpoint", type=Path, required=True, help="原始训练检查点路径")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("checkpoints/cmm_crosssim_write_noise_conditions"),
        help="CMM-on-CrossSim write_noise 条件模型输出目录",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def build_condition_models(args: argparse.Namespace) -> List[Dict[str, str]]:
    """按 write_noise_std 和 seed 逐个构建 CMM-on-CrossSim checkpoint。"""
    ...
    for write_noise_std in write_noise_stds:
        ...
        for seed in seeds:
            ...
            # CrossSim programming_error 在构建 AnalogLinear 时注入并随 checkpoint 固化。
            ...
            torch.save(
                {
                    ...
                    "crosssim_programming_error_std": write_noise_std,
                    "seed": seed,
...
def main() -> None:
    """主入口：构建 CMM-on-CrossSim write_noise 条件模型并输出 manifest。"""
    rows = build_condition_models(parse_args())
    print("=== Built CMM-on-CrossSim write_noise condition models ===")
    for row in rows:
        print(
