"""
批量构建不同 ADC 分辨率下的 CrossSim 条件模型。

默认只改变 ADC，DAC 固定在中等非理想基线（12 bit）。
如需隔离纯 ADC，请显式传入 --dac-resolution 0。
"""

# 运行示例（Windows，使用 ^ 续行）：
#   :: 默认 ADC 分辨率扫描（12, 10, 8, 6, 4 bit），含理想基线
#   python -m src.test_crosssim_adc_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_adc_conditions --save-baseline-crosssim
#
#   :: 自定义 ADC 分辨率集合，跳过已存在的文件
#   python -m src.test_crosssim_adc_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_adc_conditions ^
#       --resolutions 10,8,6 --skip-existing
#
#   :: GPU 加速，配合固定 DAC=4 bit 做 ADC/DAC 交叉消融
#   python -m src.test_crosssim_adc_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_adc_dac4 ^
#       --device cuda --use-gpu --dac-resolution 4 --resolutions 10,8,6,4
#
#   :: 小 tile + 单元量化 + 自定义器件电阻范围
#   python -m src.test_crosssim_adc_conditions --checkpoint checkpoints\caption_transformer_epoch_10.pt ^
#       --output-dir checkpoints\crosssim_adc_small ^
#       --tile-rows 64 --tile-cols 64 --cell-bits 4 --rmin 500 --rmax 200000
...
def parse_args() -> argparse.Namespace:
    """解析 ADC 分辨率扫描参数。"""
    parser = argparse.ArgumentParser(description="Build CrossSim checkpoints for ADC resolution sweep")
    parser.add_argument("--checkpoint", type=Path, required=True, help="原始训练检查点路径")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("checkpoints/crosssim_adc_conditions"),
        help="ADC 条件模型输出目录",
    )
    parser.add_argument(
        "--device",
    ...
    parser.add_argument("--dac-resolution", type=int, default=12, help="固定 DAC 分辨率；0 表示理想 DAC")
    ...
    parser.add_argument(
        ...
        type=str,
        default="12,10,8,6,4",
        help="逗号分隔的 ADC bit 列表，例如 12,10,8,6,4",
    )
    parser.add_argument(
        "--save-baseline-crosssim",
        action="store_true",
        help="同时保存 ADC=0、DAC=固定值的 CrossSim 基准模型",
    )
...
def parse_resolutions(raw: str) -> List[int]:
    ...
    if not values:
        raise ValueError("--resolutions 至少需要一个 ADC bit 值。")
...
def build_condition_models(args: argparse.Namespace) -> List[Dict[str, str]]:
    """按 ADC 分辨率逐个构建 CrossSim checkpoint。"""
    ...
    # 组装条件列表：可选基线 + 扫描的 ADC 分辨率
    conditions: List[tuple[str, int]] = []
    if args.save_baseline_crosssim:
        conditions.append(("baseline-crosssim", 0))
    conditions.extend((f"adc-{bits}", bits) for bits in parse_resolutions(args.resolutions))

    rows: List[Dict[str, str]] = []
    for condition, adc_bits in conditions:
        ...
        print(f"[build] {condition} adc_resolution={adc_bits}, dac_resolution={args.dac_resolution}")
        t0 = time.time()

        # 以当前 ADC 分辨率进行 CrossSim 映射
...
def main() -> None:
    """主入口：构建 ADC 分辨率条件模型并输出 manifest。"""
    rows = build_condition_models(parse_args())
    print("=== Built CrossSim ADC condition models ===")
    for row in rows:
        print(
