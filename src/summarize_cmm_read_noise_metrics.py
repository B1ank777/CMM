"""
按 read_noise_std 汇总 CMM 多 seed 评估结果，输出 mean ± std。

输入应为 src.compute_metrics_pycoco 生成的 metrics JSON；其中每个 CMM 条件行
需要包含 read_noise_std、seed 和 metrics 字段。
"""

# 运行示例：
#   python -m src.summarize_cmm_read_noise_metrics ^
#       --metrics checkpoints\cmm_read_noise_conditions\metrics_pycoco.json ^
#       --output checkpoints\cmm_read_noise_conditions\metrics_mean_std.json
...
def parse_args() -> argparse.Namespace:
    """解析 mean ± std 汇总参数。"""
    parser = argparse.ArgumentParser(description="Summarize CMM read_noise metrics by mean ± std")
    parser.add_argument("--metrics", type=Path, required=True, help="compute_metrics_pycoco 输出的 metrics JSON")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("checkpoints/cmm_read_noise_conditions/metrics_mean_std.json"),
...
def summarize(metrics_payload: Dict[str, Any], metric_keys: List[str]) -> List[Dict[str, Any]]:
    """按 read_noise_std 聚合同一噪声强度下的多 seed 指标。"""
    groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for row in metrics_payload.get("results", []):
        if row.get("model") == "baseline":
            continue
        if "read_noise_std" not in row or "seed" not in row:
            continue
        groups[str(row["read_noise_std"])].append(row)

    summaries: List[Dict[str, Any]] = []
    for read_noise_std in sorted(groups.keys(), key=lambda value: float(value)):
        rows = groups[read_noise_std]
...
def main() -> None:
    ...
    for item in summaries:
        print(f"read_noise_std={item['read_noise_std']} (n={item['n']}, seeds={','.join(item['seeds'])})")
        for key in metric_keys:
